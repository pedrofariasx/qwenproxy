...
const SEND_SELECTORS = ['.message-input-right-button-send .send-button', '.chat-prompt-send-button', 'button.send-button'];
...
export async function loginToQwen(email: string, password: string): Promise<boolean> {
  ...
  const errorSelector = '[class*="error"], [class*="Error"], [role="alert"], .ant-form-item-explain-error';
...
export async function loginToQwenViaApi(email: string, password: string): Promise<boolean> {
  if (!activePage) throw new Error('Playwright not initialized');
  ...
  const result = await activePage.evaluate(async ({ email, password, timezone }) => {
    try {
  ...
  if (result.ok && result.data?.success) {
    ...
    if (!activePage.url().includes('auth') && !activePage.url().includes('login')) {
      console.log('[API] Login OK');
      return true;
    }
  }

  const msg = result.data?.data?.details || result.data?.message || result.error || 'Login failed';
...
async function _fetchQwenHeaders(forceNew = false): Promise<{ headers: Record<string, string>, chatSessionId: string, parentMessageId: string | null }> {
  ...
  await activePage.waitForSelector(INPUT_SELECTOR, { timeout: 30000 }).catch(() => {
